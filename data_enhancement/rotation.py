...
def rotate_image(self):
        ...
        """
        height, width = self.image.shape[:2]  #Make sure that the image has only 2 dimensions
        image_center = (width / 2,
                        height / 2)  # get image center for cv2 rotation function
...
def yoloFormattocv(x1, y1, x2, y2, H, W):
    ...
    voc.append(center_x - (box_w / 2))
    voc.append(center_y - (box_h / 2))
    voc.append(center_x + (box_w / 2))
    voc.append(center_y + (box_h / 2))
...
def cvFormattoYolo(corner, H, W):
    box_W = corner[3] - corner[1]
    box_H = corner[4] - corner[2]

    center_box_x = (corner[1] + corner[3]) / 2
    center_box_y = (corner[2] + corner[4]) / 2

    return corner[0], round(center_box_x / W, 6), round(center_box_y / H, 6), round(box_W / W, 6), round(box_H / H,6)